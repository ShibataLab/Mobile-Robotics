...
#include <octomap_process/Num.h>
#include <octomap_process/hmm_srv.h>
	...
	if(cloud_seq_loaded.size()==2){
		...
		{	
			...
			if (prev_node!= NULL && curr_node!=NULL )
			{
				...
				ros::ServiceClient client = k.serviceClient<octomap_process::hmm_srv>("hmm_data");
				//octomap_process::Num message;
				//message.num1 = curr_node->getOccupancy();
				//message.num2 = prev_node->getOccupancy();
				//pub.publish(message);
				//rate.sleep();
				octomap_process::hmm_srv srv;
